...
def frequency_blur_channel(img: np.ndarray, psf: np.ndarray) -> np.ndarray:
    ...
    H_pad = np.zeros((h, w), dtype=np.float32)
    
    # Handle case where PSF might be larger than image
    cy_start = max(0, (h - ph) // 2)
    cx_start = max(0, (w - pw) // 2)
    cy_end = min(h, cy_start + ph)
    cx_end = min(w, cx_start + pw)
    
    psf_y_start = max(0, (ph - h) // 2)
    psf_x_start = max(0, (pw - w) // 2)
    psf_y_end = psf_y_start + (cy_end - cy_start)
    psf_x_end = psf_x_start + (cx_end - cx_start)
    
    H_pad[cy_start:cy_end, cx_start:cx_end] = psf[psf_y_start:psf_y_end, psf_x_start:psf_x_end].astype(np.float32)
    H_for_fft = np.fft.ifftshift(H_pad)
    
    # FFT on original image size (no padding)
...
def frequency_blur_color(img_rgb: np.ndarray, psf: np.ndarray, gamma: float = 2.2, show_intermediate: bool = False) -> np.ndarray:
    ...
    if show_intermediate:
        ...
        H_pad[cy_start:cy_end, cx_start:cx_end] = psf[psf_y_start:psf_y_end, psf_x_start:psf_x_end]
        H_for_fft = np.fft.ifftshift(H_pad)
        ...
        plt.subplot(2, 4, 6); plt.imshow(img_chan, cmap='gray'); plt.title('Padded Input'); plt.axis('off')
...
def frequency_blur_color_debug(img_rgb: np.ndarray, psf: np.ndarray, gamma: float = 2.2) -> np.ndarray:
    img_lin = gamma_correction(img_rgb, 1.0 / gamma)
    out = np.zeros_like(img_lin)
    for c in range(3):
        out[..., c] = frequency_blur_channel(img_lin[..., c], psf)
...
if __name__ == '__main__':
    ...
    spatial_times = []
    freq_times = []
    sizes = [350, 300, 250, 200, 150, 120, 100, 80, 60, 40, 30, 20, 10]
    psfs = resize_and_normalize_psfs(base_star, sizes)
    kernel_sizes = [psf.shape[0] for psf in psfs]
        

    # Timing on impulse image
    for psf in psfs:
        t0 = time.time()
    ...
    plt.loglog(kernel_sizes, spatial_times, 'o-', label='Impulse: Spatial Domain (conv)')
    plt.loglog(kernel_sizes, freq_times, 's-', label='Impulse: Frequency Domain (FFT)')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PART_D, 'timing_loglog_impulse.png'), bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    print('All timing outputs saved under', PART_D)
