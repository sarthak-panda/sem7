...
#     mag_display = np.log1p(mag) # using log for display
...
#     ax.set_title("Select spectral peaks (left-click). Press ENTER when done.", fontsize=10)
#     ax.axis('off')
#     picked = []  # store (x, y) tuples in axis/data coords

#     def onclick(event):
#         # left mouse button only, and inside axes
...
#             ax.plot(x, y, marker='+', color='r', markersize=8, mew=1.5)
#             fig.canvas.draw()
...
#     # Convert picked (x=col, y=row) coords to (row, col) and sample intensity
#     for (x, y) in picked:
#         # round to nearest pixel and make sure indices are in bounds
#         cxp = int(round(x))
#         ryp = int(round(y))
#         cxp = int(np.clip(cxp, 0, W-1))
#         ryp = int(np.clip(ryp, 0, H-1))
#         # compute small neighbourhood average (3x3) for robustness
...
# Make sure this global is defined at module top-level:
# AUTOMATED_PEAK_DETECTOR = True  # or False for manual selection
...
def detect_spectral_peaks(mag: np.ndarray, exclude_radius: int = 10, top_percentile: float = 99.5,
    ...
    # Mouse click: add pick (left button)
    def onclick(event):
        nonlocal drag_mode
        if event.inaxes is None:
            return
        # left-click to add a pick (only when not in drag-mode)
        # If in drag-mode, we treat motion events for adjusting; left-click still appends a new pick
        if event.button == 1:
            x, y = event.xdata, event.ydata
            if x is None or y is None:
                return
    ...
    # Mouse move: if in drag mode, update last pick using current pointer
    def onmove(event):
        if not drag_mode:
            return
        if event.inaxes is None:
        ...
        if x is None or y is None:
            return
        # update the picked list and visuals for the last element
        picked[-1] = (x, y)
    ...
    # Convert picked (x=col, y=row) coords to (row, col) and sample intensity (3x3 avg)
    centers = []
    for (x, y) in picked:
        cxp = int(round(x))
        ryp = int(round(y))
